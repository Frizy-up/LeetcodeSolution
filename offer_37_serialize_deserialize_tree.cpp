...
public:
	...
	vector<string> parse_string(const string& data)
	{
		vector<string> ret;
		string temp = "";
		for (auto &it : data)
	...
	{
		vector<string> vec_str = parse_string(data);
		if (vec_str.size() == 0) return NULL;
		TreeNode* root = new TreeNode(atoi(vec_str[0].c_str()));
		queue<TreeNode*> q;
		q.push(root);
		int j = 1;
		for (int i = 1; i<vec_str.size(); i++)
		{
			if (!q.empty())
			{
				TreeNode* tmp = q.front();
				q.pop();
				if (j<vec_str.size() && vec_str[j] != "null")
				{
					tmp->left = new TreeNode(atoi(vec_str[j++].c_str()));
					q.push(tmp->left);
				}
				else
				{
				...
				if (j<vec_str.size() && vec_str[j] != "null")
				{
					tmp->right = new TreeNode(atoi(vec_str[j++].c_str()));
					q.push(tmp->right);
				}
				else
				{
